...
local fh = assert(io.open("C:/_PROYECTOS/Cidelsa/debug/destryer/dumps/io_trace_altair_mame.txt","w"))
local cnt = 0
local irqs = 0
local prevP = 0
...
emu.register_frame_done(function()
    if cnt >= N then fh:close(); print(">>> io_trace_altair_mame.txt: "..cnt.." OUTs"); m:exit() end
end)
