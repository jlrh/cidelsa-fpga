-- captura genérica Cidelsa (Destroyer/Altair): VRAM+config+screen.bin de una escena.
-- Config de DISPLAY CORRECTA: snapshot de reg_disp en la 1ª lectura de PAGE RAM del
-- CDP1869 de cada frame (= instante de RENDER del device = config real de display).
-- No depende de lecturas de CPU (que no disparan en todos los juegos).
local TARGET=tonumber(os.getenv("CIDELSA_FRAME")) or 1000
local DIR=(os.getenv("CIDELSA_DIR") or "C:/_PROYECTOS/Cidelsa/debug/destryer/altair").."/"
...
TAPS[#TAPS+1]=io_sp:install_write_tap(0x03,0x07,"o",function(o,d,mk)
  ...
  if n==3 then local x=d&0xff; reg.bkg=x&7; reg.cfc=(x>>3)&1; reg.dispoff=(x>>4)&1; reg.col=(x>>5)&3; reg.freshorz=(x>>7)&1
    ...
    elseif n==6 then reg.pma=a&0x7ff elseif n==7 then reg.hma=a&0x7fc end
  end
end)
-- 1ª lectura de PAGE RAM por frame = RENDER del CDP1869 → snapshot config de display
...
TAPS[#TAPS+1]=prog:install_write_tap(0xF400,0xF7FF,"c",function(off,d,mk)
  ...
  local pmd=pageram:read_u8(pma&0x7ff); local column=((pma&0x400)~=0) and 0xff or pmd
  local addr=((column<<3)|(cma&7))&0x7ff; charram[addr]=d&0xff; pcbram[addr]=qv() end)
...
emu.register_frame_done(function()
  ...
  local page={}; for i=0,0x3ff do page[i]=pageram:read_u8(i) end
  dh(DIR.."page_ram.hex",page,0x400); dh(DIR.."char_ram.hex",charram,0x800); dh(DIR.."pcb_ram.hex",pcbram,0x800)
  local fh=assert(io.open(DIR.."regs.txt","w")); for _,k in ipairs({"bkg","cfc","dispoff","col","freshorz","line9","line16","dblpage","fresvert","hma"}) do fh:write(string.format("%s=%d\n",k,reg_disp[k])) end; fh:close()
  ...
  print(string.format(">>> %s f%d DISPLAY: dbl=%d l9=%d l16=%d fhz=%d fvt=%d col=%d cfc=%d bkg=%d hma=0x%x",
     DIR,f,reg_disp.dblpage,reg_disp.line9,reg_disp.line16,reg_disp.freshorz,reg_disp.fresvert,reg_disp.col,reg_disp.cfc,reg_disp.bkg,reg_disp.hma)); m:exit()
end)
